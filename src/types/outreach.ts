...
/* ai_site_findings_v2's name lives in one leaf (src/lib/siteFindings.ts, beside the generator for
   the {{6}} that makes it different). */
import { AI_SITE_FINDINGS_V2 } from '@/lib/siteFindings';
...
export const WHATSAPP_TEMPLATES: { value: string; label: string }[] = [
  ...
  /* initial_opener_v2 — the A/B variant of the opener, submitted to Meta 2026-09-22.
     ...
     nothing in the SPA named the template, so a row already sent with it would print on the
     dashboard as the raw key. Every sendable template must be labelled in exactly one list, and
  ...
  /* ai_site_findings_v2 — APPROVED BY META 2026-09-25 in its edited form: SIX variables ({{1}} trade,
     {{2}} town, {{3}}-{{5}} rivals, {{6}} what AI is seeing on their site) and NO report link — the
     report stays on the audit for a follow-up once they reply.
     ⛔ LISTED UNCONDITIONALLY, for the reason template-picker-parity.test.ts enforces: a row already
     sent with a template must always have a label. getTemplateSendability is still what decides
     whether THIS lead can be sent it ({{6}} needs a value). */
  { value: AI_SITE_FINDINGS_V2, label: 'AI site findings — rivals + what AI is seeing on their site, no link (outreach)' },
  ...
  { value: 'explain_offer', label: 'Explain the offer — BLOCKED: quotes the old £29.99/month (re-register at Meta)' },
  ...
  { value: 'explain_offer_v2', label: 'Explain the offer v2 (adds the 941-audit proof) — BLOCKED: quotes the old £29.99/month (re-register at Meta)' },
