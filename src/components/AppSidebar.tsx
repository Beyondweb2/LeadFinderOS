...
import { 
  ...
  ChevronLeft, ChevronRight, Briefcase, DollarSign,
  HelpCircle, Users, MessageSquare, ShieldCheck
} from 'lucide-react';
        ...
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <div className="overflow-hidden">
              ...
              <p className="text-xs text-sidebar-foreground/60 truncate">All-in-one CRM</p>
            </div>
          )}
        </div>
      </SidebarHeader>
      ...
      <SidebarContent className="p-2">
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {navItems.map((item) => {
                ...
                const isFlashing = (item.url === '/outreach' && flashCRM) || (item.url === '/potential-work' && flashTrack);
        ...
        {isAdmin && (
          <SidebarGroup>
            <SidebarGroupContent>
              {!isCollapsed && (
                <div className="px-3 py-2 text-xs font-semibold text-sidebar-foreground/50 uppercase tracking-wider">Admin</div>
              )}
              <SidebarMenu>
                {adminItems.map((item) => {
                  const isActive = location.pathname === item.url;
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className={cn('flex items-center', isCollapsed ? 'justify-center' : 'justify-between')}>
          ...
          <div className="flex items-center gap-1">
            <AccentColorPicker />
            <Button variant="ghost" size="icon" onClick={toggleSidebar} className="h-8 w-8 text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent">
