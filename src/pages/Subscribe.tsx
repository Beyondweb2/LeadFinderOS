...
const Subscribe = () => {
  ...
  const { createCheckout, subscribed, isLoading: subLoading, isPaidSubscriber, isStripeTrialing, checkSubscription } = useSubscription();
    ...
    try {
      ...
      if (data?.url) {
        ...
        pollRef.current = setInterval(async () => {
          try { await checkSubscription(true); } catch {}
        }, 3000);
      }
    } catch (error) {
        ...
        <Card className="bg-card/80 backdrop-blur-xl border-primary/20">
          <CardHeader className="text-center pb-2">
            ...
            <CardDescription className="text-base">
              Get unlimited searches and all features
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex flex-col gap-3">
            {waitingForPayment ? (
              <div className="w-full text-center space-y-3">
                ...
                <p className="text-xs text-muted-foreground">Complete checkout in the new tab.</p>
                <Button variant="outline" size="sm" onClick={() => { setWaitingForPayment(false); if (pollRef.current) { clearInterval(pollRef.current); pollRef.current = null; } }}>
                  Cancel
                </Button>
              </div>
            ) : (
              <>
                <Button onClick={handleSubscribe} className="w-full" size="lg" disabled={isLoading}>
                  {isLoading ? (
                    <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Starting...</>
                  ) : (
                    <><CreditCard className="mr-2 h-4 w-4" />Unlock unlimited — £19.99/month</>
                  )}
                </Button>
