      ...
      <section className="relative z-10 py-10 sm:py-14 md:py-20 px-3 sm:px-4">
        <div className="container mx-auto max-w-5xl">
          <ScrollReveal className="text-center mb-10 sm:mb-12 md:mb-16">
            ...
            <p className="text-sm sm:text-lg md:text-xl text-foreground/60 max-w-lg mx-auto leading-relaxed mt-4 sm:mt-5">
              Find your target audience and reach them directly instead of waiting for inbound traffic.
            </p>
          </ScrollReveal>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6 md:gap-8 max-w-4xl mx-auto">
            {[
              { icon: Search, title: 'Find your audience', text: 'Discover businesses that match your ideal customer.' },
              { icon: MessageSquare, title: 'Reach decision-makers', text: 'Contact potential users directly with high reply rates.' },
              { icon: Zap, title: 'Track conversations', text: 'Manage follow-ups and conversations in one place.' },
            ].map((item, i) => (
              <ScrollReveal key={i} delay={i * 100}>
                <div
                  className="flex flex-col items-center text-center p-6 sm:p-8 rounded-2xl h-full"
                  style={{
                    background: 'linear-gradient(135deg, hsl(var(--card)) 0%, hsl(var(--card)/0.8) 100%)',
                    border: '1px solid hsl(var(--border)/0.5)',
                  }}
                >
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4" style={{ background: 'hsl(var(--primary)/0.1)' }}>
                    <item.icon className="h-5 w-5 text-primary" />
                  </div>
                  <h3 className="text-base sm:text-lg font-semibold text-foreground mb-2">{item.title}</h3>
                  <p className="text-xs sm:text-sm text-foreground/55 leading-relaxed">{item.text}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
