...
import {
  Search, ClipboardList, Phone, FileText, Zap, Check, ArrowRight, X, Volume2, VolumeX, MessageSquare, Star, ChevronLeft, ChevronRight
} from 'lucide-react';
        ...
        <div className="container mx-auto max-w-6xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div className="space-y-3">
              <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">The Old Way</p>
              <div className="rounded-xl overflow-hidden border border-white/10 shadow-lg">
                <img src={oldWayImage} alt="The old way — scrolling Google Maps" className="w-full h-auto object-contain" />
              </div>
            </div>
            <div className="space-y-3">
              <p className="text-sm font-medium text-primary uppercase tracking-wider">The New Way</p>
          ...
          {isMobile ? (
            <div className="relative">
              ...
              <div className="flex items-center justify-center gap-4 mt-4">
                <button
                  onClick={() => setToolkitIndex((prev) => (prev - 1 + FEATURES.length) % FEATURES.length)}
                  className="h-9 w-9 rounded-full border border-border/40 bg-card/60 flex items-center justify-center hover:bg-card transition-colors"
                >
                  <ChevronLeft className="h-5 w-5 text-foreground" />
                </button>
                <span className="text-xs text-muted-foreground">{toolkitIndex + 1} / {FEATURES.length}</span>
                <button
                  onClick={() => setToolkitIndex((prev) => (prev + 1) % FEATURES.length)}
                  className="h-9 w-9 rounded-full border border-border/40 bg-card/60 flex items-center justify-center hover:bg-card transition-colors"
                >
      ...
      <section className="relative z-10 py-16 sm:py-24 px-4">
        <div className="container mx-auto max-w-3xl text-center">
          <ScrollReveal>
            ...
            <Button
              size="lg"
              className="font-semibold px-8 h-[52px] rounded-xl text-base"
              style={{ background: 'hsl(142 70% 45%)', color: 'white' }}
              asChild
            >
              <a href="https://wa.me/447477932564" target="_blank" rel="noopener noreferrer">
