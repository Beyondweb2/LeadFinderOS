/* ============================================================
   ...
   ============================================================ */
import { canDeriveReport, explainRefusal, nameIsJudgeable, MIN_ANSWERED_DATAPOINTS } from "../supabase/functions/_shared/derivable.ts";
...
console.log("\n── nameIsJudgeable — THE NAME CLAUSE ALONE, FOR A SCORE THAT ALREADY EXISTS ──");
/* ⛔ IT MUST AGREE WITH canDeriveReport ON THE NAME AND DISAGREE ON THE SAMPLE SIZE. A trade-level
   fold counts stored `named` flags, so "did we look hard enough" has already been answered by the
   audit existing; re-asking it would delete every 3-question hook (6 cells) from every figure. */
{
  ok(!nameIsJudgeable({ businessName: "Blackpool Plumber", trade: "plumbers", town: "Blackpool" }),
     '"Blackpool Plumber" is not judgeable — its name IS the question');
  ok(!nameIsJudgeable({ businessName: "Chichester Accountants Ltd", trade: "accountants", town: "Chichester" }),
     '"Chichester Accountants Ltd" is not judgeable');
  ok(!nameIsJudgeable({ businessName: "  ", trade: "plumbers", town: "Blackpool" }),
     "a blank name is not judgeable — there is nothing to have matched");
  ok(nameIsJudgeable({ businessName: "Lockwood & Sons", trade: "locksmiths", town: "Norwich" }),
     '"Lockwood & Sons" IS judgeable');
  /* The clause that must NOT apply: two engines on a 3-question hook is 6 answered cells. */
  ok(canDeriveReport({ businessName: "Lockwood & Sons", trade: "locksmiths", town: "Norwich", answeredDatapoints: 6 }).reason === "too_few_answers",
     "canDeriveReport still refuses that name at 6 datapoints (the derivation rule is unchanged)");
  ok(nameIsJudgeable({ businessName: "Lockwood & Sons", trade: "locksmiths", town: "Norwich" }),
     "  … and nameIsJudgeable still says yes, because it asks only about the name");
}
