...
console.log("\n── ⛔ THE COMBINED FILTER COVERS EVERY STATUS, EXACTLY ONCE ──");
{
  ...
  /* ⚠️ COUNT UPDATED 2026-08-23 with the 'already_visible' status (+1). It was ALSO stale before this
     (it read 17 while the list had grown to 18 with in_delivery/completed) — this Deno suite is not in
     the npm pipeline, so nobody had re-run it. Now: 19 statuses; no_whatsapp + no_whatsapp_needs_sms
     share a label → 18 groups; 18 groups + the Paid sentinel = 19 filter options. */
  ok(OUTREACH_STATUS_FILTER_OPTIONS.length === 19 && OUTREACH_STATUS_OPTIONS.length === 19,
    `18 status groups + the Paid sentinel = 19 filter options for 19 statuses (got ${OUTREACH_STATUS_FILTER_OPTIONS.length} for ${OUTREACH_STATUS_OPTIONS.length})`);
