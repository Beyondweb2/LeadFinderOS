...
console.log("── Branch 3 (the fix): website but no scan is STATED, not silent ──");
{
  const html = renderReportHtml({ ...base, seo: undefined, hasWebsite: true });
  ok(html.includes("full check comes when we start work"), "the sequencing line renders");
  ok(html.includes("This report measures whether AI names you today"), "and says what THIS report is");
  ok(!html.includes("How findable is your website?"), "no graded panel without a grade");
  ok(!html.includes("You don&rsquo;t have a website yet"), "and never the no-website copy for a business that has one");
}
