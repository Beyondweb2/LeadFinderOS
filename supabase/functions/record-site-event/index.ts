...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (event === "open") {
      // Track the open only: set first_opened_at once, bump open_count, log the event.
      // These drive the Opened pill/filter. Opening no longer creates a follow-up TASK
      // on the lead — a follow-up is set when the lead REPLIES (whatsapp-inbound), not
      // when they merely open the link.
