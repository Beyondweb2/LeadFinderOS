-- Add the newly-approved WhatsApp template "booking_switch_barbers" to the Inbox
...
-- from 20260706120000_mirror_add_barber_poor_website.sql VERBATIM, adding ONLY the
-- new `when 'booking_switch_barbers'` branch before the `else` — the existing
-- booking_page_intro / no_website_barbers / barber_poor_website branches and
-- everything else are unchanged. The existing trigger (trg_mirror_whatsapp_send)
-- references this function by name, so replacing the function is enough; the trigger
